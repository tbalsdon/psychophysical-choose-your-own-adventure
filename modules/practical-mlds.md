...
In this example we will work with the data from Knoblauch & Maloney (2008). This data comes
from a tutorial about the use of MLDS and the implementation of an R-package for data
analysis, which I thoroughly recommend reading (reference below). The focus of this practical
will be concerned with model-fitting more generally.
...
perceived correlation increases non-linearly with r. How to objectively measure/model
this?
...
1. **run:** There are three runs, labelled kk1, kk2, kk3, each run the observer performs one
...
6. **S4:** Stimulus number for the fourth stimulus (d) - 1 to 11, see key above.
7. **invord:** Whether the pair order was actually reversed (`[a,b]` > `[c,d]`).
...
2. It's difficult to think of a good way to visualise this data, because there are many
   ...
   differences `[|(a-b)|-|(c-d)|]` (sort the pairs such that a<b<c<d - you can swap the pair
   order according to invord, column 7); the y axis will be the proportion of second pair
   choices; plot different lines for the value of `[a]`, you may need to bin this too (average
   of 2 or 3 values of `[a]`; you wont have data for all points on the x-axis for all lines).
   What you should see is that as the value of `[a]` decreases, the lines become flatter: The
   perceived difference in differences increases with increasing values.
3. Define a model that describes how the observer is performing this task: for each external
   stimulus, they have some internal value, which is disrupted by (additive Gaussian) noise.
   The observer takes the difference in differences `[|(a-b)|-|(c-d)|]` and reports whether
   this was less than 0 (the second difference was larger). This model therefore has 12
   parameters: the number of means (11 internal values), and the standard deviation of the
   additive Gaussian noise. To simplify things in our model, we can assumed the noise is
   independent, such that the variance adds across the 4 stimuli, meaning the standard
   deviation parameter actually describes the total noise SD on each trial. As a first step,
   one could simulate this model (taking 1000 samples of noise per trial, adding this to the
   difference in differences, and then comparing this value to 0; the tutorial
   [Psychometrics → Types of decisions → Detection: 2AFC](psy-detection-2afc.md) is one
   example of this). Check how the model responds by averaging the simulations and plotting
   the same way as you did at step 2 (set the initial means to the stimulus values, you could
   also try some exponential of this, or approximate the values from Figure 1 of Knoblauch &
   Maloney, 2008).
4. Fit the model: for this, the fastest way is to compute the likelihood directly, and add
   ...
   observer chose the first interval, the log likelihood is the cumulative normal of the
...
5. What you probably just did was fit 12 parameters as instructed. Choose different starting
   parameters (multiply everything by 10). You might find different best fitting parameters.
   Plot the model with these parameters, you will probably get the same plot. This is because
   the parameters are poorly constrained: there are multiple solutions. We fix this problem by
   setting the mean of the first stimulus value to 0 and the mean of the largest stimulus
   value to 1. You should now have 10 free parameters.
...
7. How many trials would you need to fit this model? Run a parameter recovery analysis,
   simulating the model with different numbers of trials (if the fitting process doesn't take
   too long). One issue with standard parameter recovery analyses (where you simulate trials
   from the model and then fit these trials) is that it assumes all variability in your actual
   data comes from variability captured by the model. This is fine if what we want to know is
   simply: what is the noise limit and is the model identifiable? But in this situation, we
   have massively simplified our model to make it an imperfect but useful description of the
   underlying processes (the transformation to internal values might not be quite an
   exponential, for example), so we might assume there is more variance in real data than in
   the simulated model. To assess how well this model might describe real data, we can use a
   bootstrapping approach: Take random permutations of trials from the actual data, and re-fit
   the model (if simulating the same number of trials, some will be randomly omitted and
   others randomly repeated). Bootstrap over a smaller number of trials and calculate the
   variability in the recovered parameters. How does the number of trials affect the
   reliability of the model parameters (at what point do you start getting very different
   predictions of behaviour from that sampled from this same individual)? (A simpler
   demonstration of the effect of the number of trials is given in
